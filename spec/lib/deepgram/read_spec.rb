...
RSpec.describe Deepgram::Read::Client do
  describe '#summarize' do
    ...
    let(:fixture) { Deepgram::Fixtures.load_file('pencils_summary.json')}
    ...
    it 'sends a POST request with the text as the body' do
      ...
      expect(res.summary).to eq("The speaker discusses the history and characteristics of the Fascinating World ofGeneration, a world filled with technological innovations. They explore the world of pencils, including the origins of the name Fascinating World of Nosils and the artistry involved in making them. They also mention the importance of the graphite and clay components in making a great pencil and encourage viewers to take a moment to appreciate its rich history and artistry.")
  ...
  describe '#topics', :focus do
    ...
    it 'sends a POST request with the text as the body' do
      ...
      expect(res.topics.first).to eq({
        "text" => "The Origins of the Pencil The pencil's origins can be traced back to ancient times when the Romans and Egyptians used a form of lead ore known as plumbago to mark and write. However, it wasn't until the late 16th century that the modern pencil began to take shape.",
        "start_word" => 49,
        "end_word" => 97,
        "topics" => [
          {
  ...
  describe '#sentiment' do
    ...
    it 'sends a POST request with the text as the body' do
      ...
      expect(res.sentiments.first).to eq({
        "text" => "Let's Explore the Fascinating World of Pencils In a world filled with technological marvels, it's easy to overlook the humble pencil. Yet, this unassuming writing instrument has a rich history and a surprising depth of complexity.",
        "start_word" => 0,
        "end_word" => 35,
        "sentiment" => "neutral",
